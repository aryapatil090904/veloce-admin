...
import { createMember, getWaiverStatus } from "../members/_api/members";
...
export default function OnboardingPage() {
  ...
  // Periodically check backend API for waiver completion when waiting
  useEffect(() => {
    if (waiverStatus !== "waiting") return;

    const checkBackendStatus = async () => {
      const res = await getWaiverStatus(waiverId);
      if (res && res.status === "signed") {
        setWaiverStatus("signed");
        setCustomerSignature(res.signature || "");
        if (res.photoUrl) {
    ...
    checkBackendStatus();
    const interval = setInterval(checkBackendStatus, 1500);
    return () => clearInterval(interval);
  }, [waiverId, waiverStatus]);
  ...
  const handleCompleteRegistration = async () => {
    ...
    if (!fullName) {
      setErrorMsg("First Name or Last Name is required");
      return;
    }
    if (!email) {
    ...
    try {
      const created = await createMember({
        ...
        email,
        phone,
        tier: selectedTierData.tierValue,
        status: "Active",
        avatarUrl: photoUploaded ? photoUrl : "",
        lastCheckInDate: "Just now",
        lastCheckInLocation: "Downtown Hub",
      });

      if (created) {
        setSuccessMsg("Member registered successfully! Redirecting to Directory...");
        setTimeout(() => {
          router.push("/members");
        }, 1500);
      } else {
        setErrorMsg("Failed to register member. Please check backend server.");
      }
    } catch (err) {
  ...
  return (
    <>
      <div className="max-w-[1600px] mx-auto animate-in fade-in zoom-in-95 duration-700 relative">
        ...
        {errorMsg && (
          <div className="mb-6 p-4 rounded-xl bg-error/10 border border-error/30 text-error font-label text-sm flex items-center gap-2">
        ...
        {successMsg && (
          <div className="mb-6 p-4 rounded-xl bg-secondary/10 border border-secondary/30 text-secondary font-label text-sm flex items-center gap-2">
              ...
              <div className="glass-card bg-surface-container/60 p-8 rounded-xl border border-outline-variant/10 shadow-[0_0_40px_rgba(0,0,0,0.3)]">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      className="w-full bg-surface-container-low border border-outline-variant/20 rounded-lg px-4 py-3 focus:outline-none focus:ring-1 focus:ring-primary/50 focus:border-primary/50 transition-all text-on-surface placeholder:text-outline/50" 
                  ...
                  <div className="col-span-2 space-y-2">
                    ...
                    <input 
                      className="w-full bg-surface-container-low border border-outline-variant/20 rounded-lg px-4 py-3 focus:outline-none focus:ring-1 focus:ring-primary/50 focus:border-primary/50 transition-all text-on-surface placeholder:text-outline/50" 
                      placeholder="m.aurelius@empire.fit" 
                      type="email" 
                      value={email}
                  ...
                  <div className="col-span-2 border-t border-outline-variant/10 pt-4 mt-2">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                      <div className="space-y-2">
                        ...
                        <input 
                          className="w-full bg-surface-container-low border border-outline-variant/20 rounded-lg px-4 py-3 focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all text-on-surface placeholder:text-outline/50" 
                          placeholder="e.g. 75" 
                          type="number" 
                          value={startWeight}
              ...
              <div className="glass-card bg-surface-container-high/80 p-6 rounded-xl border border-outline-variant/20 sticky top-28">
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 border border-outline-variant/30 bg-surface-container-high flex items-center justify-center">
                      {photoUrl ? (
                  ...
                  <button 
                    disabled={waiverStatus !== "signed" || isSubmitting}
